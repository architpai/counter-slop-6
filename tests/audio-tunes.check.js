...
export function run(assert) {
  ...
  const notes = (key, index, intensity = 0) => {
    const events = [];
  ...
  const has = (events, expected) => events.some((event) => Object.entries(expected).every(([key, value]) => typeof value === 'number' ? near(event[key], value) : event[key] === value));
    ...
    for (const intensity of [0, 1]) {
      for (let index = 0; index < tune.steps; index++) {
        for (const event of notes(key, index, intensity)) {
          assert(Number.isFinite(event.freq) && event.freq > 0, `${key}/${index}: frequency`);
          assert(Number.isFinite(event.duration) && event.duration > 0, `${key}/${index}: duration`);
          assert(Number.isFinite(event.gain) && event.gain > 0 && event.gain <= 1, `${key}/${index}: gain`);
          assert(Number.isFinite(event.at) && event.at >= 10, `${key}/${index}: time`);
          assert(event.channel === 'noise' ? event.end === undefined : Number.isFinite(event.end) && event.end > 0, `${key}/${index}: sweep`);
        }
      }
    }
