...
function mesh(parent, geo, color, x = 0, y = 0, z = 0, unlit = false) {
...
function oval(parent, color, x, y, z, rx, ry, rz) {
...
function face(parent, radius, smile) {
...
function hat(parent, kind, color, size) {
...
function weaponProp(kind, color) {
  const root = new THREE.Group();
  root.name = kind;
  const addBox = (w, h, d, x, y, z, c = color) => mesh(root, boxGeo(w, h, d), c, x, y, z);
  const barrel = (r, length, x, y, z) => mesh(root, cylGeo(r, length), DARK, x, y, z);
  if (kind === 'blade') {
...
export function makeWeaponProp(index) {
  return weaponProp(['rifle', 'shotgun', 'sniper', 'blade'][index] ?? 'rifle', TONE_HEX[TONE.HOSTILE]);
}
...
export function makeFigure(o) {
  ...
  let eyeSets, weapon = null;
  const joint = (name, parent, x, y, z) => (parts[name] = group(parent, name, x, y, z));
  const anchor = (name, parent, x = 0, y = 0, z = 0) => (anchors[name] = group(parent, `${name}Hit`, x, y, z));
  const legs = (hips, width, thighLength, shinLength, radius, offset) => {
  ...
  if (o.kind === 'flyer') {
    ...
    if (o.blob === 'hitbox') {
      ...
      parts.spikes = group(torso, 'spikes');
      for (let i = 0; i < 9; i++) {
        const a = i / 9 * Math.PI * 2;
        const spike = mesh(parts.spikes, coneGeo(0.12, 0.42, 5), color,
    ...
    parts.gunMount = group(parts.foreR, 'gunMount', 0, -0.29, 0.07);
    if (o.shield) {
