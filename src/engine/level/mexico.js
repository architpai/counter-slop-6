...
export function buildMexico(b) {
  ...
  for (let i = -2; i <= 1; i++) {
    const q = 24 * i + 12;
    for (const [x, z] of [[q, -57], [q, 57], [-57, q], [57, q]]) b.marker('spawns', x, 0, z);
  }
  ...
  for (const [x, z, r] of [[-52, -46, 2.2], [52, 48, 2.6], [-52, 48, 1.8],
    [52, -48, 2], [0, -52, 1.6], [0, 52, 1.6]]) {
    b.sphere(x, 0.55 * r, z, r);
    b.collider(x, 0, z, 1.5 * r, 1.2 * r, 1.5 * r);
  }
  for (const [x, z, w, d] of [[-12, -12, 8, 0.5], [12, -12, 8, 0.5],
    [-30, 34, 0.5, 8], [30, 34, 0.5, 8]]) b.box(x, 0, z, w, 1.1, d, ORANGE);
  b.cylinder(-14, 0, 8, 1.3, 1);
  b.box(-14, 1, 8, 0.15, 2, 0.15, DARK_VISUAL);
  b.box(-14, 3, 8, 2.2, 0.3, 0.3, DARK_VISUAL);

  for (const p of [[-10, 30.6, 46], [10, 15, 46], [-40, 7.5, 14], [40, 7, -18],
    [0, 5.9, -26], [0, 13.45, 0]]) b.marker('snipers', ...p);
  for (const p of [[22, 0, 6.5], [26, 0, 6.5], [24, 0, 1.5], [0, 1.25, 8],
    [-20, 0, 0], [20, 0, -14], [0, 0, -36], [-40, 6, -20], [40, 5.5, 0],
    [0, 11, 44], [0, 13.5, 0], [-24, 0, 24], [24, 0, 24]]) b.marker('pickups', ...p);
  for (const p of [[-40, 6.2, -20], [40, 7.2, -18], [-40, 7.7, 14], [40, 6.7, 16],
    [0, 11.2, 44], [0, 5.6, -26], [-46, 0, 0], [46, 0, 0], [0, 0, -50],
    [-30, 0, 46], [30, 0, 46], [0, 13.6, 0], [-10, 30.8, 46]]) b.marker('arenaSpawns', ...p);
  ...
  for (const [x, z, w, h] of [[-120, -160, 60, 30], [40, -190, 90, 36],
    [150, -120, 70, 26], [-170, 60, 50, 24], [160, 90, 80, 30], [-60, 190, 100, 34]]) {
...
function bandstand(b) {
  ...
  for (const [x, z, yaw, trumpet] of [[-2.6, -27.5, 0.4, false], [0, -28.5, 0, true],
    ...
    b.addObject(root);
    b.level.animated.push({ mesh: root, update(t) {
...
function church(b) {
  for (const box of [[0, 0, 44, 24, 11, 16], [0, 11, 44, 24, 1.6, 4.5],
    ...
    [10, 0, 46, 6, 15, 6], [10, 20.8, 46, 0.3, 2, 0.3]]) b.box(...box);
...
function market(b) {
  for (const [x, z, w, d] of [[-20, 22, 4.5, 2.4], [-13, 22, 4.5, 2.4],
    [20, 22, 4.5, 2.4], [13, 22, 4.5, 2.4], [-24, -12, 2.4, 4.5], [26, -8, 2.4, 4.5]]) {
  ...
  for (const [x, y, z] of [[-20, 1.3, 22], [13, 1.3, 22], [-24, 1.3, -12], [26, 1.3, -8],
    [0, 6.4, 8], [-9, 9.5, 12], [9, 9.5, 12]]) prop(b, 'pinata', x, y, z);
  for (const [x, z] of [[-17.5, 24.5], [-9.5, 24.5], [16.5, 24.5], [23.5, 24.5],
    [-27.5, -9], [-27.5, -15], [29, -5], [29, -11]]) prop(b, 'crate', x, 0, z);
  for (const [x, z] of [[-33, -14], [-33, -12.6], [-33, -6], [-33, 10], [-33, 20], [33, -12],
    [33, -3], [33, 6], [33, 22], [-6, 30], [6, 30], [-18, 31], [18, 31], [-3, -33], [3, -33]])
    prop(b, rand() < 0.3 ? 'potL' : 'potS', x, 0, z);
  for (const [x, z] of [[-18, -30], [18, -30], [-30, 30], [30, 30]]) prop(b, 'barrel', x, 0, z);
  for (const [x, z, h] of [[-46, -40, 2.8], [-50, -28, 2.2], [-48, 30, 3], [-44, 44, 2.4],
    [46, -44, 2.6], [50, -30, 2.2], [48, 34, 3.2], [44, 46, 2.5], [-30, -48, 2.8],
    [30, -48, 2.4], [-28, 48, 2.6], [28, 50, 2.9], [12, -44, 2.2], [-12, -44, 2.6]])
    prop(b, 'cactus', x, 0, z, h);
}

function prop(b, kind, x, y, z, height) {
  const parts = [];
  const box = (w, h, d, px, py, pz, surface) => parts.push(part(new THREE.BoxGeometry(w, h, d), px, py, pz, surface));
  const cylinder = (rt, rb, h, px, py, pz, surface, seg = 8, rotZ = 0) =>
    parts.push(part(new THREE.CylinderGeometry(rt, rb, h, seg).rotateZ(rotZ), px, py, pz, surface));
  const torus = (r, tube, py, surface) =>
    parts.push(part(new THREE.TorusGeometry(r, tube, 6, 12).rotateX(HALF_PI), 0, py, 0, surface));
  let w, h, d, hp = 1, tone = TONE.ACCENT;
...
function part(geometry, x, y, z, surface) {
