...
export function buildDowntown(b) {
  ...
  ].map(team => team.map(point => new THREE.Vector3(...point)));
}
...
function buildArena(b) {
  ...
  for (const values of [[-8, 0, 20, 3, 1, 1.2], [10, 0, 26, 1.4, 1.2, 1.4],
    [-12, 0, -8, 2.4, 0.8, 2.4], [14, 0, -4, 2.4, 0.8, 2.4]]) b.box(...values);
  ...
  const shell = { noNav: true, noGrapple: true };
...
function buildTower(b) {
  ...
  for (const [x, z] of [[-6.6, -6.6], [6.6, -6.6], [-6.6, 6.6], [6.6, 6.6],
    [0, -6.6], [0, 6.6], [-6.6, 0], [6.6, 0]]) b.box(x, 0, z, 0.8, 16, 0.8);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const [x1, x2] = even ? [1.3, 3.5] : [-7.2, -5];
...
function buildA(b) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const [x1, x2] = even ? [-37, -34.8] : [-28.5, -26.3];
...
function buildB(b) {
  for (const rectangle of [[24, 4, 44, 9], [24, 15, 44, 20], [24, 9, 31, 15], [37, 9, 44, 15]]) {
  ...
  for (const rectangle of [[24.4, 4.4, 26, 19.6], [42, 4.4, 43.6, 19.6],
    [26, 4.4, 42, 6], [26, 18, 42, 19.6]]) b.slab(...rectangle, 6, 0.3);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const [x1, x2] = even ? [33.8, 36] : [25.3, 27.5];
    b.slab(x1, 20.2, x2, 24.4, y + 4, 0.4);
    b.rail(x1, 24.4, x2, 24.4, y + 4);
  }
...
function buildHouses(b) {
  for (const [x, h] of [[-30, 7], [-8, 11], [16, 7]]) b.box(x, 0, -45, 14, h, 10);
...
function buildPlaza(b) {
  ...
  for (const [x, z] of [[-10, 46], [10, 46], [-22, 24], [22, 24]]) {
  ...
  for (const values of [[-16, 0, -8, 2.2, 1.2, 2.2], [18, 0, -10, 2.2, 1.6, 2.2],
    [-20, 0, 8, 1.6, 1, 3], [20, 0, -2, 3, 1, 1.6], [-8, 0, -18, 4, 1.1, 1.2],
    [8, 0, -18, 4, 1.1, 1.2], [0, 0, 22, 5, 0.5, 1.4]]) b.box(...values);
...
function buildSky(b) {
  ...
  for (const [x, y, z, s] of [[60, 70, -170, 1], [-20, 75, -190, 1.3], [140, 60, -80, 0.9],
    [-150, 65, 40, 1.1], [30, 80, 180, 1.2], [-90, 60, 170, 0.8]]) {
    for (let i = 0; i < 6; i++) {
