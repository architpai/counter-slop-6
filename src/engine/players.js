...
const LIMBS = ['upperL', 'upperR', 'foreL', 'foreR', 'thighL', 'thighR', 'shinL', 'shinR'];
...
function triple(arr, start = 0, limit = 10000) {
  return bounded(arr[start], limit) && bounded(arr[start + 1], limit) && bounded(arr[start + 2], limit);
}

function validState(arr) {
  return Array.isArray(arr) && [8, 11, 14].includes(arr.length) && triple(arr) &&
    Number.isFinite(arr[3]) && bounded(arr[4], 1.6) && Number.isInteger(arr[5]) &&
    Number.isInteger(arr[6]) && arr[6] >= 0 && arr[6] <= 511 &&
    Number.isFinite(arr[7]) && arr[7] >= 0 && arr[7] <= 120 &&
...
export class RemotePlayer {
  constructor(ctx, id, name, team = 0, tone = TONE.HOSTILE) {
    this._ctx = ctx;
    this._id = id;
    this.name = name;
    this.team = team;
    ...
    this.hits = Object.entries(HIT_RADII).map(([part, r]) => ({ part, r, obj: new Object3D() }));
    this.lastSeen = 0;
    this.deadT = 0;
    this.onDamage = null;
    ...
    this._buildFigure();
    this._figure.root.visible = false;
    this._derive();
    this._placeHits();
  }
  ...
  set name(value) { this._name = cleanName(value); }
  ...
  _buildFigure() {
    this._figure = makeFigure({ kind: 'humanoid', color: TONE_HEX[this._tone], scale: 1,
      bodyWidth: 1, headSize: 1, limbR: 0.033, hat: 'cap', smile: false, shield: false, weapon: 'rifle' });
    const { anchors, root } = this._figure;
    for (const part of ['armL', 'armR']) anchors[part].position.y = -0.15;
    for (const part of ['foreL', 'foreR']) anchors[part].position.y = -0.14;
    for (const part of ['legL', 'legR', 'shinL', 'shinR']) anchors[part].position.y = -0.21;
    this._wi = -1;
    this._flashT = 0;
  ...
  _removeTag() {
    ...
    this._tag.userData.dispose();
    this._tag = null;
  }

  _makeTag() {
    this._removeTag();
    this._tag = makeNameTag(this.name);
    this._tagName = this.name;
    this._tag.position.y = 2.25;
    this._figure.root.add(this._tag);
  }
  ...
  push(arr, now) {
    ...
    if (this._figure && (wi !== this._wi || !this._figure.parts.weapon?.parent)) {
      this._figure.setWeapon(WEAPONS[wi]);
      this._wi = wi;
    }
    const flags = arr[6];
    ...
    this.hp = arr[7];
    if (arr.length >= 11) this.body.vel.set(arr[8], arr[9], arr[10]);
    else this.body.vel.set(0, 0, 0);
    if (this.grappling) this.hook.set(arr[11], arr[12], arr[13]);
  ...
  _pose(dt) {
    const { root, parts: p } = this._figure;
    root.position.copy(this.body.pos);
  ...
  _placeHits() {
    ...
    for (const hit of this.hits) {
      if (this._figure) this._figure.anchors[hit.part].getWorldPosition(hit.obj.position);
  ...
  shots(kind, ends) {
    if (this._disposed || !this.alive || !this._figure || typeof kind !== 'string' || kind.length > 32 ||
      !Array.isArray(ends) || !ends.length || ends.length > 90 || ends.length % 3 !== 0) return;
    ...
    for (let i = 0; i < ends.length; i += 3) {
      endpoint.set(ends[i], ends[i + 1], ends[i + 2]);
  ...
  ragdoll(dir, over) {
    if (this._disposed || !this._figure || typeof over !== 'boolean' ||
      !(dir === null || (Array.isArray(dir) && dir.length === 3 && triple(dir, 0, 1)))) return;
    const d = dir ? new Vector3(...dir) : new Vector3();
    ...
    if (over) {
      const detach = (part, extra, radius) => {
        const obj = figure.parts[part];
    ...
    for (const part of LIMBS) {
      figure.parts[part].rotation.x = rand(-1.2, 1.2);
      figure.parts[part].rotation.z = rand(-0.6, 0.6);
    }
